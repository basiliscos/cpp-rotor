...
template <typename T, typename M, typename Marker> struct forwarder_t {
    forwarder_t(T &obj_, M &&member_, Marker &&marker_)
        : obj{obj_}, impl_fn{std::forward<M>(member_)}, marker{std::forward<Marker>(marker_)} {}

    template <typename E, typename... R> void operator()(const E &err, R... results) noexcept {
        if (err) {
...
template <typename T, typename M> struct forwarder_t<T, M, void> {
    forwarder_t(T &obj_, M &&member_) : obj{obj_}, impl_fn{std::forward<M>(member_)} {}
...
template <typename T, typename M> forwarder_t(T, M)->forwarder_t<std::decay_t<T>, M, void>;

template <typename T, typename M, typename Marker>
forwarder_t(T, M, Marker)->forwarder_t<std::decay_t<T>, M, std::decay_t<Marker>>;

class my_simple_agent_t {
  public:
    my_simple_agent_t(const std::string &host_, const std::string proto_, boost::asio::io_context &io_context_)
        : host{host_}, proto{proto_}, io_context{io_context_}, resolver{io_context}, skt{io_context} {}
    ...
    void resolve() noexcept {
        ...
        resolver.async_resolve(host, proto, forwarder_t(*this, &my_simple_agent_t::on_resolve, ctx_resolve{}));
    }

    void on_resolve(boost::asio::ip::tcp::resolver::results_type &&results) noexcept {
        std::cout << "on_resolve()\n";
        boost::asio::async_connect(skt, results.begin(), results.end(),
...
class my_simple_agent2_t {
  public:
    my_simple_agent2_t(const std::string &host_, const std::string proto_, boost::asio::io_context &io_context_)
        : host{host_}, proto{proto_}, io_context{io_context_}, resolver{io_context}, skt{io_context} {}

    void on_error(const boost::system::error_code &ec) { std::cout << "on_error2 [generic] " << ec.message() << "\n"; }

    void resolve() noexcept {
        std::cout << "resolve2()\n";
        resolver.async_resolve(host, proto, forwarder_t(*this, &my_simple_agent2_t::on_resolve));
    }
...
class my_simple_agent3_t {
  public:
    my_simple_agent3_t(const std::string &host_, const std::string proto_, boost::asio::io_context &io_context_)
        : host{host_}, proto{proto_}, io_context{io_context_}, resolver{io_context}, skt{io_context} {}
    ...
    void resolve() noexcept {
        std::cout << "resolve3()\n";
        resolver.async_resolve(host, proto, forwarder_t(*this, &my_simple_agent3_t::on_resolve, "resolve"));
    }
