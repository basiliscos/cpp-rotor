...
struct my_supervisor_t : public rotor::supervisor_t {
    my_supervisor_t(rotor::system_context_t &ctx, const rotor::supervisor_config_t &config)
        : rotor::supervisor_t{ctx, config} {}

    ~my_supervisor_t() override { std::cout << "my_supervisor_t::~my_supervisor_t\n"; }
...
struct pinger_t : public rotor::actor_base_t {
    ...
    void on_initialize(rotor::message_t<rotor::payload::initialize_actor_t> &) noexcept override {
